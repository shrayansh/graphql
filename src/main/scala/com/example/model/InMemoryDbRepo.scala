...
import com.example.common.JsonProtocol._
...
class InMemoryDbRepo(db: Database)(implicit ec: ExecutionContext) extends BookRepo with AuthorRepo {
  ...
  def authors(ids: Seq[String]) = {
    println(s"Executing SQL Query:$ids")
...
object InMemoryDbRepo {
  ...
  class AuthorTable(tag: Tag) extends Table[Author](tag, "AUTHORS") {
    ...
    def * = (id, name, bio) <> ((Author.apply _).tupled, Author.unapply)
  }
...
//  }
  ...
  def createDatabase(implicit ec: ExecutionContext) = {
    ...
    val books = Source.fromResource("bookData/books.json").getLines().mkString.parseJson.convertTo[List[Book]]
    val authors = Source.fromResource("bookData/authors.json").getLines().mkString.mkString.parseJson.convertTo[List[Author]]
